...
from app.schemas.endpoint import EndpointCreate,EndpointResponse,EndpointResult
...
@router.get("/", response_model=list[EndpointResult])
...
async def run_check(id:int, db:Session= Depends(get_db),user:User= Depends(get_current_user)):
    endpoint=db.query(Endpoint).filter(Endpoint.id== id,Endpoint.user_id==user.id).first()
